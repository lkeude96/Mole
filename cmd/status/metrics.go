...
type MetricsSnapshot struct {
	...
	Bluetooth      []BluetoothDevice `json:"bluetooth_devices"`
...
type DiskIOStatus struct {
	ReadRate  float64 `json:"read_rate_mbs"`  // MB/s
	WriteRate float64 `json:"write_rate_mbs"` // MB/s
}
...
type MemoryStatus struct {
	...
	Cached      uint64  `json:"cached_bytes"` // File cache that can be freed if needed
...
type ProxyStatus struct {
	...
	Type    string `json:"type"` // HTTP, SOCKS, System
	Host    string `json:"host"`
}
...
type ThermalStatus struct {
	...
	SystemPower  float64 `json:"system_power_w"`  // System power consumption in Watts
	AdapterPower float64 `json:"adapter_power_w"` // AC adapter max power in Watts
	BatteryPower float64 `json:"battery_power_w"` // Battery charge/discharge power in Watts (positive = discharging)
}
...
func (c *Collector) Collect() (MetricsSnapshot, error) {
	...
	mergeCollectErr := func(err error) {
		if err == nil {
			return
		}
		errMu.Lock()
	...
	collect := func(fn func() error) {
		...
		go func() {
			...
			defer func() {
				if r := recover(); r != nil {
					mergeCollectErr(fmt.Errorf("metric collection panicked: %v", r))
				}
			}()
			if err := fn(); err != nil {
				mergeCollectErr(err)
